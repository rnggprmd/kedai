...
@section('page-subtitle', 'Perbarui kapasitas, kode, atau status meja.')
...
@section('content')
<div class="max-w-2xl -mt-4 lg:-mt-6">
    <div class="bg-white p-8 lg:p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
        <form action="{{ route('admin.tables.update', $table) }}" method="POST" class="space-y-8">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <input type="text" name="kode_meja" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 font-bold text-slate-900 focus:ring-2 focus:ring-brand-primary transition-all placeholder:text-slate-300 @error('kode_meja') ring-2 ring-brand-primary @enderror" value="{{ old('kode_meja', $table->kode_meja) }}" required>
                ...
                <div>
                    ...
                    <input type="text" name="nama_meja" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 font-bold text-slate-900 focus:ring-2 focus:ring-brand-primary transition-all placeholder:text-slate-300 @error('nama_meja') ring-2 ring-brand-primary @enderror" value="{{ old('nama_meja', $table->nama_meja) }}" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <input type="number" name="kapasitas" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 font-bold text-slate-900 focus:ring-2 focus:ring-brand-primary transition-all" value="{{ old('kapasitas', $table->kapasitas) }}" min="1" required>
                </div>
                ...
                <div class="bg-slate-50 p-5 rounded-2xl flex items-center justify-between border border-slate-100">
                    ...
                    <div class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ $table->is_active ? 'checked' : '' }} class="sr-only peer">
