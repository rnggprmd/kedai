...
@section('page-subtitle', 'Masukkan detail menu baru yang ingin ditambahkan.')
        ...
        <form action="{{ route('admin.menus.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 font-bold text-slate-900 focus:ring-2 focus:ring-brand-primary transition-all placeholder:text-slate-300 @error('nama') ring-2 ring-brand-primary @enderror" placeholder="e.g. Special Fried Rice" value="{{ old('nama') }}" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="harga" class="w-full bg-slate-50 border-none rounded-2xl pl-12 pr-5 py-4 font-bold text-slate-900 focus:ring-2 focus:ring-brand-primary transition-all @error('harga') ring-2 ring-brand-primary @enderror" placeholder="25000" value="{{ old('harga') }}" required>
                    </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 font-medium text-slate-600 focus:ring-2 focus:ring-brand-primary transition-all" rows="3" placeholder="Ingredients, taste, etc...">{{ old('deskripsi') }}</textarea>
                </div>
            ...
            <div class="flex gap-4 pt-4 border-t border-slate-100">
                <button type="submit" class="flex-1 bg-brand-primary text-brand-secondary font-extrabold py-5 rounded-2xl hover:opacity-90 transition-all shadow-xl shadow-brand-primary/20">
                    Create Menu Item
                </button>
