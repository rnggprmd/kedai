...
@section('page-subtitle', 'Update the name or grouping of your menu items.')
    ...
    <div class="bg-white p-8 lg:p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-8">
            ...
            <div>
                ...
                <input type="text" name="nama" class="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 font-bold text-slate-900 focus:ring-2 focus:ring-indigo-600 transition-all" value="{{ old('nama', $category->nama) }}" required>
            </div>
